...
export default function MatchOutline({ onScore }: MatchOutlineProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-4 w-full">
      <p className="text-white font-extrabold text-xl">
        Match the outline! Find the solid shape. 🔍
      </p>
      ...
      >
        <ShapeDisplay shape={target} size={110} outline />
        <p className="text-white/60 text-sm font-bold">Find this shape below</p>
      </motion.div>
        ...
        {choices.map((s) => (
          <motion.button
            ...
            className={`rounded-2xl p-4 flex flex-col items-center gap-1 shadow-lg
              ${feedback === "correct" && s.name === target.name
