...
import Shapes3DGame from "@/components/shapes3d/Shapes3DGame";
import { useSession } from "@/hooks/useSession";

export default function Shapes3DPage() {
  const [showBurst, setShowBurst] = useState(false);
  useSession("shapes3d" as never);
  ...
  return (
    <main className="h-screen w-screen bg-gradient-to-b from-violet-500 to-purple-800 flex flex-col overflow-hidden game-area">
      <div className="flex items-center justify-center pt-4 pb-2 px-4">
        <h1 className="text-3xl font-black text-white drop-shadow">🔷 3D Shapes</h1>
      </div>
      <p className="text-center text-white/70 text-xs pb-2">Spheres, cubes, cones and more!</p>
      <div className="flex-1 overflow-auto">
        <Shapes3DGame onScore={(c, t) => {
