...
import PluralsGame from "@/components/plurals/PluralsGame";
import { useSession } from "@/hooks/useSession";

export default function PluralsPage() {
  const [showBurst, setShowBurst] = useState(false);
  useSession("plurals" as never);
  ...
  return (
    <main className="h-screen w-screen bg-gradient-to-b from-orange-400 to-red-600 flex flex-col overflow-hidden game-area">
      <div className="flex items-center justify-center pt-4 pb-2 px-4">
        <h1 className="text-3xl font-black text-white drop-shadow">🐱🐱 Plurals</h1>
      </div>
      <p className="text-center text-white/70 text-xs pb-2">One or many? Learn how words change!</p>
      <div className="flex-1 overflow-auto">
        <PluralsGame onScore={(c, t) => {
