...
import MoreLess from "@/components/moreorless/MoreLess";
import { useSession } from "@/hooks/useSession";

export default function MoreOrLessPage() {
  const [showBurst, setShowBurst] = useState(false);
  useSession("moreorless" as never);
  ...
  return (
    <main className="h-screen w-screen bg-gradient-to-b from-green-500 to-teal-700 flex flex-col overflow-hidden game-area">
      <div className="flex items-center justify-center pt-4 pb-2 px-4">
        <h1 className="text-3xl font-black text-white drop-shadow">⚖️ More or Less</h1>
      </div>
      <p className="text-center text-white/70 text-xs pb-2">Compare groups — more, less, or equal?</p>
      <div className="flex-1 overflow-auto">
        <MoreLess onScore={(c, t) => {
